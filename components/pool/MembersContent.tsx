...
export function MembersContent({ poolId }: { poolId: string }) {
          ...
          <div className="bg-[#141414] border border-[#2a2a2a] rounded-2xl px-4">
            {canReorder ? (
              <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
                <SortableContext items={activeMembers.map((m) => m._id)} strategy={verticalListSortingStrategy}>
                  {activeMembers.map((m) => (
              ...
              activeMembers.map((m) => (
                <MemberRow key={m._id} name={m.displayName ?? ""} email={m.email} status={m.status} />
              ))
            )}
          </div>
        </div>
